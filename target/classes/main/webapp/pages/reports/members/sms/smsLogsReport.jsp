...
<c:if test="${queryExecuted}">
	...
	<c:choose><c:when test="${empty smsLogs}">
		<div class="footerNote"><bean:message key="smsLog.search.noResults"/></div>
	</c:when><c:otherwise>
		<table class="defaultTableContent" cellspacing="0" cellpadding="0">
		    <tr>
		            ...
		            <table class="defaultTable" cellspacing="0" cellpadding="0">
		            			...
		            			<table>
									...
									<c:forEach var="type" items="${typesList}">
			            				<tr>
											...
											<c:forEach var="status" items="${statusList}">
												<td align="right"><cyclos:format number="${totals[type][status]}"/></td>
											</c:forEach>
											<td align="right"><b><cyclos:format number="${totalsByType[type]}"/></b></td>
			            				</tr>
									</c:forEach>
									<tr>
		            					<td class="headerLabel"><b><bean:message key="global.total"/></b></td>
										<c:forEach var="status" items="${statusList}">
											<td align="right"><b><cyclos:format number="${totalsByStatus[status]}"/></b></td>
										</c:forEach>
										<td align="right"><b><cyclos:format number="${total}"/></b></td>
									</tr>
		            			</table>
						...
						<c:forEach var="log" items="${smsLogs}">
			                <tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
			                    ...
			                    <td align="center"><a class="profileLink" style="text-decoration:underline" memberId="${log.targetMember.id}">${log.targetMember[ownerProperty]}</a></td>
			                    <td align="center">			
