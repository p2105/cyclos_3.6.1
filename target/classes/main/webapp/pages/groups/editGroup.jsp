		       		...
		       		<fieldset>
						...
						<table class="defaultTable">
			                ...
			                <tr>
			                    ...
			                    <td>
			                    	<html:select property="group(memberSettings).emailValidation" styleClass="InputBoxDisabled" disabled="true">
			                    		<c:forEach var="emailValidation" items="${emailValidations}">
											<html:option value="${emailValidation}"><bean:message key="group.settings.emailValidation.${emailValidation}"/></html:option>
			                    		</c:forEach>
			                    	</html:select>
			                    </td>
			                </tr>
					...
					<c:if test="${showScheduling}">
						...
						<fieldset>
							...
							<table class="defaultTable">
				                ...
				                <tr>
				                    <td class="label"><bean:message key="group.settings.maxSchedulingPeriod"/></td>
				                    <td>
				                ...
				                <tr>
				                    <td class="label"><bean:message key="group.settings.showPosWebPaymentDescription"/></td>
...
<c:if test="${!isInsert && isMemberGroup && group.status.enabled}">
	...
	<c:choose>
		<c:when test="${empty accountTypes}">
			<div class="footerNote"><bean:message key="group.account.noResults"/></div>
		</c:when>
		<c:otherwise>
			<table class="defaultTableContent" cellspacing="0" cellpadding="0">
			    <tr>
...
<c:if test="${!isInsert && group.status.enabled}">
	...
	<c:choose>
		<c:when test="${empty customizedFiles}">
			<div class="footerNote" title="<bean:message key="group.customizedFiles.title"/>"><bean:message key="group.customizedFiles.noResults"/></div>
		</c:when>
		<c:otherwise>
			<table class="defaultTableContent" cellspacing="0" cellpadding="0">
			    <tr>
