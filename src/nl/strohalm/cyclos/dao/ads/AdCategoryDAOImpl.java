...
public class AdCategoryDAOImpl extends BaseDAOImpl<AdCategory> implements AdCategoryDAO {
    ...
    public List<AdCategory> searchLeafAdCategories(final AdCategoryQuery query) {
        ...
        // It's not easy to find an order clause in SQL - we're sorting by full name, which mixes different hierarchy levels.
        // So, we just fetch all leaf categories and apply sort in-memory
        Collections.sort(categories, new BeanComparator("fullName"));
        return categories;
    }
