...
public class PaymentServiceImpl implements PaymentServiceLocal {
    ...
    @Override
    public void processScheduled(final Period period, final boolean notify) {
        ...
        try {
            for (final Transfer transfer : transfers) {
                processScheduledTransfer(transfer, true, notify, notify);
     ...
     */
    @Override
    public void validateMaxAmountToday(final Account account, final TransferType transferType, BigDecimal maxAmountPerDay, final BigDecimal amount) {
        ...
        if (maxAmountPerDay != null && maxAmountPerDay.floatValue() > PRECISION_DELTA) {
            ...
            BigDecimal amountOnDay = transferDao.getTransactionedAmountToday(account, transferType);
        ...
        if (LoggedUser.hasUser() && LoggedUser.isOperator()) {
            ...
            if (maxAmount != null && maxAmount.floatValue() > PRECISION_DELTA) {
                // Get the amount on today
                BigDecimal amountOnDay = transferDao.getTransactionedAmountToday(operator, account, transferType);
                // Validate
     ...
     */
    private Payment doInsert(final TransferDTO dto, final AuthorizationLevel firstAuthorizationLevel, final boolean newTransaction, final boolean simulation) {
        return transactionHelper.maybeRunInNewTransaction(new Transactional<Payment>() {
            ...
            @Override
            public Payment doInTransaction(final TransactionStatus status) {
                return performInsert(dto, firstAuthorizationLevel, simulation);
            }
        }, newTransaction);
    }
    ...
    private Payment doPayment(final DoPaymentDTO params, final boolean newTransaction, final boolean notify, final boolean simulation) {
        ...
        final Payment payment = doInsert(dto, firstAuthorizationLevel, newTransaction, simulation);
    ...
    private Transfer doProcessScheduledTransfer(final LockHandler lockHandler, Transfer transfer, final boolean failOnError, final boolean notifyPayer, final boolean notifyReceiver) {
        ...
        try {
            ...
            validateMaxAmountToday(from, transferType, null, amount);
            ...
            // Only generate a member notification if at least receiver is notified
            if (notifyReceiver) {
    ...
    private AuthorizationLevel firstAuthorizationLevel(TransferType transferType, final BigDecimal amount, AccountOwner from) {
        ...
        if (transferType.isRequiresAuthorization() && CollectionUtils.isNotEmpty(transferType.getAuthorizationLevels())) {
            ...
            BigDecimal amountSoFarToday = transferDao.getTransactionedAmountToday(account, transferType);
    ...
    private Payment insert(final TransferDTO dto, final boolean newTransaction, final boolean simulation) {
        ...
        return doInsert(dto, firstAuthorizationLevel, newTransaction, simulation);
    }
    ...
    private Transfer insertTransferAndPayFees(final LockHandler lockHandler, Transfer transfer, final boolean forced, final boolean simulation, final Set<ChargedFee> chargedFees) {
        ...
        if (transfer.getProcessDate() == null) {
            ...
            accountService.reservePending(transfer);
        } else if (transfer.getProcessDate().before(transfer.getDate())) {
            // For payments in past, make sure that closed balances after the past date are removed (so they can be correctly closed in future dates)
     ...
     */
    private Payment performInsert(final LockHandler lockHandler, final TransferDTO dto, final AuthorizationLevel firstAuthorizationLevel, final boolean simulation) {
        ...
        if (CollectionUtils.isEmpty(dto.getPayments())) {
            ...
            // Lock the source account if it is limited
            if (applicationService.getLockedAccountsOnPayments() == LockedAccountsOnPayments.ORIGIN && fromAccount.getCreditLimit() != null) {
            ...
            if (transferToProcess != null) {
                doProcessScheduledTransfer(transferToProcess, true, false, true);
            }
     ...
     * Locks the accounts which need to be locked, and perform the insert
     */
    private Payment performInsert(final TransferDTO dto, final AuthorizationLevel firstAuthorizationLevel, final boolean simulation) {
        LockHandler lockHandler = lockHandlerFactory.getLockHandlerIfLockingAccounts();
        try {
            return performInsert(lockHandler, dto, firstAuthorizationLevel, simulation);
        } finally {
            if (lockHandler != null) {
                lockHandler.release();
            }
    ...
    private void verify(final TransferDTO params) {
        ...
        validateMaxAmountToday(fromAccount, transferType, null, amount);
