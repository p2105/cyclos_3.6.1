...
public class EditCustomFieldAction extends BaseFormAction {
    ...
    protected void prepareForm(final ActionContext context) throws Exception {
        ...
        if (id <= 0L) {
            ...
            if (parent == null) {
                ...
                if (parentValueId > 0L) {
                    ...
                    if (parent != null && CollectionUtils.isNotEmpty(parent.getPossibleValues())) {
                        parentValue = parent.getPossibleValues().iterator().next();
                        form.setParentValueId(parentValue.getId());
                    }
                }
                possibleValues = field.getPossibleValuesByParent(parentValue);
            }
        ...
        switch (nature) {
            case MEMBER:
                // View may be any access level
                RequestHelper.storeEnum(request, MemberCustomField.Access.class, "accessForView");
                // Edit access level can be any but OTHER
                request.setAttribute("accessForEdit", EnumSet.complementOf(EnumSet.of(MemberCustomField.Access.OTHER)));
