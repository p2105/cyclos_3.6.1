	        ...
	        <td colspan="2" align="left" class="tdContentTableLists">
	            <table class="defaultTable">
					...
					<c:forEach items="${indexesStatus}" var="entry">
						...
						<c:set var="canOptimize" value="${cyclos:name(status) == 'NOT_OPTIMIZED'}" />
		                <tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
		                    ...
		                    <c:if test="${!allOptimized}">
			                    <td align="center">
			                    <c:if test="${canOptimize}">
				                    <a class="default optimize" entityType="${type}">
				                    	<bean:message key="adminTasks.indexes.optimize" />
				                    </a>
			                    </c:if>
			                    </td>
		                    </c:if>
	...
	<table class="defaultTableContentHidden" cellspacing="0" cellpadding="0">
		<tr>
			<td align="right" nowrap="nowrap">
				<c:if test="${!allOptimized}">
					<input type="button" value="<bean:message key="adminTasks.indexes.optimizeAll" />" class="button optimize">
				</c:if>
