...
<br><br>Cyclosでは、任意のCyclosシステムファイルを修正することによって、ユーザーインタフェースを変更できます。
さらに、修正を.themeファイルにパッケージ化して、他のCyclosユーザーと共有することが可能です。<br>
このことは、事実上、あなたが基本的に、あなたやあなたのメンバーがブラウザで見ることになるページについてのすべてを制御できることを意味します。つまり、コンテンツと外観は操作できます。
...
<br><br>その機能により、以下の静的ファイルの修正が可能になります（もっと多くのファイルが追加されているかもしれないため、このリストは古いかもしれません）:
<ul>
...
あなたのブラウザは、ステータスバーメッセージを隠すかもしれないことに留意して下さい。上記の場所がヘルプファイルの位置を表示しない場合は、ブラウザの設定を変更して、「ステータスバーメッセージを表示する」が設定されていることを確認してください。<br><br>あなたは、ヘルプファイルの本文内に<a href="#insert_images"><u>画像</u></a>を挿入してもよいですが、ヘルプウィンドウは300×400ピクセルしかないので、画像サイズに注意してください。
<hr class="help">
...
<br><br>問題を避けるために、Cyclosは常に容易に入れ替えられる<a
	href="#edit_customized_file"><u>元のページ</u></a>のコピーを保持しています。
<br><br>また、jspファイルのコンテンツ内に<a href="#insert_images"><u>画像</u></a>を挿入してもよいでしょう。
<hr class="help">
...
<br><br>Cyclosには以下のスタイルシートファイルがあります:
<ul>
	<li><b>style.css</b><br>
...
<br><br>CSSファイル修正の効果を見るためには、ブラウザでページを更新しなければならないかもしれません。
...
<br><br>新しいCSSファイルを使いたい場合は、（スタイルシート編集ウィンドウ内で）単にコンテンツ全体をコピーして、それを既存のスタイルシートに上書きして、上記のようにページを更新して下さい。
<br><br>素敵なCSSファイルを作り上げた場合は、是非、私たちにそれを送って下さい。
...
%&gt;</i>
<br><br>そして、画像の配置はこのようになります:
<br><br><i>&lt;img src="&lt;html:rewrite
page="/pages/images/some_image.jpg"/&gt;"&gt;</i>
<br><br>画像の配置のためにページが必要とするのは、「html:rewrite」タグのみです。
...
<br><br>このウィンドウは、<a href="#top"><u>カスタマイズ</u></a>されているファイルのリストを表示します。
あなたには以下のオプションがあります:
<ul>
...
<br><br>このウィンドウでは、あなたが以前カスタマイズしたファイルを再び修正できます。
...
<br><br>変更済みのカスタマイズしたファイルを保存する際には、元のコンテンツは保存されカスタマイズされたファイルの下に表示されます。
...
<br><br>ファイルのカスタマイズを（それをリストから除くことにより）止めると、元のコンテンツが使われます。
<hr class="help">
...
<br><br>あなたは、<img border="0"
	src="${images}/delete.gif" width="16" height="16">&nbsp;消去アイコンを選択して、画像を消去できます。<br>
下の<a href="#images_upload"><u>
更新ウィンドウ</u></a>内で、カスタム画像を追加できます。
<hr class="help">

<A NAME="style_images"></A>
<h3>スタイルシート画像</h3>
<br><br>スタイルシート画像は、ウィンドウ見出し、メニュー項目、ボタン、背景のようなCyclosのレイアウト内で使える<a href="#images"><u>画像</u></a>です。
あなたは、これらの画像を<a href="#css"><u>CSSファイル</u></a>内で使うでしょう。
<br><br><img border="0"
	src="${images}/delete.gif" width="16" height="16">&nbsp;消去アイコンを選択して、画像を消去できます。<br>
リスト内の画像サムネイルをクリックすると、ポップアップ・ウィンドウ内に実寸大の画像が表示されます。
あなたは、下の<a
	href="#images_upload"><u>更新ウィンドウ</u></a>内でカスタム画像を追加できます。
<hr class="help">

<A NAME="images_upload"></A>
<h3>画像アップロード</h3>
<br><br><a href="#system_images"><u>システム画像</u></a>の場合は、まず、「新規アップロード」ドロップダウンボックス内の上のリストから名称を選ぶことによって、どの画像を置き換えたいかを選択して下さい。
このドロップダウンは、<a
...
<h3>テーマ選択</h3>
<br><br>別の<a href="#themes"><u>テーマ</u></a>を選択するには、「テーマ」ドロップダウンボックスから１つを選び、それから、「適用する」というラベルの付いた送信ボタンをクリックしなければなりません。
...
<br><br>「削除する」というラベルの付いた送信ボタンをクリックすると、そのテーマは削除され、カスタムレイアウト修正は消去されます。
...
<br><br>この機能により、<a href="#themes"><u>テーマ</u></a>をインポートすることができます。
Cyclosのテーマは.themeという拡張子を持ちます。
ただ「ブラウズ」ボタンを使って、そのファイルの位置へとブラウズし、「送信」をクリックして下さい。
<hr class="help">

<A NAME="export_theme"></A>
<h3>現在の設定をテーマとしてエクスポートする</h3>
<br><br>あなた独自の<a href="#themes"><u>テーマ</u></a>を（<a href="#css"><u>スタイルシートファイル</u></a>および／または<a
...
<br><br><i>あなた独自のテーマを開発した場合には、それらをCyclos開発チームに送信することを検討して下さい。</i>
