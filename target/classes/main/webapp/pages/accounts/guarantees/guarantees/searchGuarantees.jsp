	    	  	...
	    	  	<table class="defaultTable">
					...
					<c:forEach var="guarantee" items="${guarantees}">
						<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
							...
							<c:if test="${showIssuer}"><td title='<cyclos:escapeHTML value="${guarantee.issuer.name}"/>'><a class="profileLink" memberId="${guarantee.issuer.id}">${guarantee.issuer.username}</a></td></c:if>
							<c:if test="${showBuyer && not query.withBuyerOnly}"><td title='<cyclos:escapeHTML value="${guarantee.buyer.name}"/>'><a class="profileLink" memberId="${guarantee.buyer.id}">${guarantee.buyer.username}</a></td></c:if>
							<c:if test="${showSeller && not query.withBuyerOnly}"><td title='<cyclos:escapeHTML value="${guarantee.seller.name}"/>'><a class="profileLink" memberId="${guarantee.seller.id}">${guarantee.seller.username}</a></td></c:if>
