						...
						<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
							...
							<c:choose>
								<c:when test="${logsBy[idx.index].byType == 'Member'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="profileLink" memberId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a></td>
								</c:when>
								<c:when test="${logsBy[idx.index].byType == 'Operator'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="operatorProfileLink" operatorId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a></td>
								</c:when>
								<c:when test="${logsBy[idx.index].byType == 'Admin'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="adminProfileLink" adminId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a></td>
								</c:when>
