        ...
        <td class="tdContentTableForms" colspan="2">
   	        <table class="defaultTable">
                <tr>
               	    ...
               	    <td><cyclos:format dateTime="${errorLogEntry.date}"/></td>
                </tr>
                <tr>
               	    ...
               	    <td>
            			<c:choose>
            				<c:when test="${not empty loggedMember}">
    	        				<a style="text-decoration:underline;" class="profileLink default" memberId="${loggedMember.id}">${loggedMember.username} - ${loggedMember.name}</a>
	            			</c:when>
	            			<c:when test="${not empty loggedAdmin}">
	            				<a style="text-decoration:underline;" class="adminProfileLink default" adminId="${loggedAdmin.id}">${loggedAdmin.username} - ${loggedAdmin.name}</a>
	            			</c:when>
	            			<c:otherwise>
