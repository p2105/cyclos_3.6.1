...
<c:if test="${queryExecuted}">
	...
	<c:choose><c:when test="${empty elements}">
		<div class="footerNote"><bean:message key="operator.search.noResults"/></div>
	</c:when><c:otherwise>
		            ...
		            <table class="defaultTable" cellspacing="0" cellpadding="0">
						...
						<c:forEach var="operator" items="${elements}">
			                <tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
			                    <td align="left" width="30%"><a class="operatorProfileLink" operatorId="${operator.id}">${operator.username}</a></td>
			                    <td align="left" width="70%"><a class="operatorProfileLink" operatorId="${operator.id}">${operator.name}</a></td>
			                </tr>
			            </c:forEach>
		            </table>
		        </td>
