...
this.options.height=this.options.height||17;addOnReadyListener(this.render.bind(this))},render:function(){var multiDropDown=this;
this.visibleRows=Math.min(this.options.size,this.values.length);this.container.setStyle({opacity:0});this.container.innerHTML="<div><input type='checkbox'>A</div>";
var height=Element.getDimensions(this.container.firstChild).height;if(height>0){this.lineHeight=height;if(is.ie){this.lineHeight+=1
}}this.container.innerHTML="";this.container.setStyle({opacity:""});var hidden=document.createElement("input");hidden.setAttribute("type","hidden");
hidden.setAttribute("name",this.name);this.valueField=this.container.appendChild(hidden);this.createHeader();this.createItemsContainer();
this.createItems();var _this=this;var name=this.name;var header=this.header;var div=this.itemsContainer;var visibleRows=this.visibleRows;
var options=this.options;var toggleFunction=function(event){var isHidden=!div.visible();if(isHidden){div.values=getValue(name);
Element.show(div);if(is.ie||is.opera){Position.prepare();var offsetTop=Element.getDimensions(header).height+document.viewport.getScrollOffsets().top;
var offsetLeft=document.viewport.getScrollOffsets().left;Position.clone(header,div,{setHeight:false,offsetTop:offsetTop,offsetLeft:offsetLeft})
}}else{Element.hide(div);if(options.onchange){var newValues=getValue(name);if(newValues&&newValues.join){newValues=newValues.join(",")
}var oldValues=div.values;if(oldValues&&oldValues.join){oldValues=oldValues.join(",")}if(newValues!=div.values){if(typeof options.onchange=="string"){eval(options.onchange)
}else{options.onchange.apply(div)}}}}if(isHidden){if(openedItemsContainer!=null){openedItemsContainer.hide()}openedItemsContainer=div;
multiDropDown.updateValues()}else{openedItemsContainer=null}if(event){Event.stop(event)}};if(!this.options.open){Event.observe(this.header,"click",toggleFunction.bindAsEventListener(this.header));
...
b.set("padding-left","4px");b.set("clear","right");b.set("height",this.options.height+"px");b.set("cursor","default");var a="multiDropDownText "+(this.options.disabled?"multiDropDownDisabled":"multiDropDown");
...
var f=this.itemsContainer;var e=this.options.singleField?"":" name='"+this.name+"'";var d=this.options.disabled;var b=this.options.open;
this.values.each(function(l,g){var h=f.appendChild(document.createElement("div"));h.style.cursor="default";var m=new StringBuffer();
m.append("<div style='white-space:nowrap'>");m.append("<input type='checkbox' class='checkbox' style='vertical-align:middle;' ").append(d?"disabled='disabled'":"").append(" value='").append(l.value).append("' ").append(e).append(l.selected?" checked='checked'":"").append(">");
m.append(" <span class='multiDropDownText' style='white-space:nowrap;padding-right:3px;vertical-align:middle;'>").append(l.text).append("</span>");
m.append("</div>");h.innerHTML+=m.toString();changeClassOnHover(h,"","multiDropDownHover");var k=h.getElementsByTagName("input")[0];
Event.observe(k,"click",function(n){if(!k.disabled){if(!b&&!a.options.disabled){a.updateValues()}if(n.stopPropagation){n.stopPropagation()
}else{n.cancelBubble=true}}});Event.observe(h,"click",function(o){if(!k.disabled){if(Event.element(o).tagName.toLowerCase()!="input"){var n=this.getElementsByTagName("input")[0];
n.checked=!n.checked}}if(!b&&!a.options.disabled){a.updateValues()}if(o.stopPropagation){o.stopPropagation()}else{o.cancelBubble=true
}}.bindAsEventListener(h))})},updateValues:function(){if(this.options.open){Element.show(this.itemsContainer)}var m="multiDropDownText "+(this.options.disabled?"multiDropDownDisabled":"multiDropDown");
...
}this.header.style.width="";this.header.innerHTML="<table style='background-color:transparent;border-spacing:0px;padding:0px;border-collapse:collapse;' cellpadding='0' cellspacing='0' height='100%'><tr><td nowrap class='multiDropDownText' style='padding:0px;padding-right:4px'><span>"+p+"</span></td><td style='padding:0px !important;' width='15' valign='top' align='right'><img style='margin:0px;width:15px;height:17px' src='"+context+"/pages/images/dropdown.gif'></td></tr></table>";
...
a=false}}if(q){o.returnValue=a;var r=q(o);if(typeof r!="undefined"){a=r}}return a}function setFocus(a){try{$(a).focus()}catch(b){}}function serializeForm(b){if(b==null){b=document.forms.length>0?document.forms[0]:null
}if(!b){var d="No form found";alert(d);throw new Error(d)}if(b.toQueryString){return b.toQueryString()}if(typeof(FCKeditorAPI)!="undefined"&&FCKeditorAPI.__Instances){for(var o in FCKeditorAPI.__Instances){try{var g=FCKeditorAPI.GetInstance(o);
g.UpdateLinkedField()}catch(n){}}}var h=[];var a=b.getElementsByTagName("*");for(var l=0;l<a.length;l++){var k=a[l];if(typeof(k.type)=="undefined"||k.name==""||k.disabled){continue
}switch(k.type){case"radio":case"checkbox":if(k.checked){h.push(k.name+"="+encodeURIComponent(k.value))}break;case"select-one":if(k.selectedIndex<0&&k.options.length>0){k.selectedIndex=0
}if(k.selectedIndex>=0){h.push(k.name+"="+encodeURIComponent(k.options[k.selectedIndex].value))}break;case"select-many":for(var f=0;
f<k.options.length;f++){var m=k.options[f];if(m.selected){h.push(k.name+"="+encodeURIComponent(m.value))}}case"file":break;
default:h.push(k.name+"="+encodeURIComponent(k.value))}}return h.join("&")}var calendarCount=0;var onCalendarUpdate=function(calendar){var toEval=calendar.params.inputField.getAttribute("onCalendarUpdate");
...
}catch(f){return null}}var g=elementId(a);var b=new FCKeditor(g);b.BasePath=context+"/pages/scripts/";b.ToolbarSet="Cyclos";
b.Config.DefaultLanguage=fckLanguage;b.ReplaceTextarea();if(is.ie){Event.observe(self,"unload",function(){a.fckEditor=null
})}a.richEditor=b;a.supportedRichEditor=!isEmpty($(elementId(a)+"___Frame"));if(!a.supportedRichEditor){a.innerHTML=replaceAll(a.value,"<br>","\n").stripTags()
}var d=a.form;Event.observe(d,"submit",function(){if(d.willSubmit&&!a.supportedRichEditor){a.value=replaceAll(a.value,"\n","<br>")
}});return b}var focusEditorOnComplete=[];function FCKeditor_OnComplete(b){var a=b.Name;if(inArray(a,focusEditorOnComplete)){var b=FCKeditorAPI.GetInstance(a);
b.Focus()}}function focusRichEditor(b){var a=getObject(b);if(a){setFocus(a)}focusEditorOnComplete.push(b);try{var d=FCKeditorAPI.GetInstance(b);
if(d==null&&a!=null){d=FCKeditorAPI.GetInstance(a.id)}d.Focus()}catch(f){}}var afterCancelEditing=null;function modifyResetClick(){this.form.reset();
disableFormFields.apply(this.form,this.form.keepFields);if(afterCancelEditing){afterCancelEditing()}}function enableFormFields(){var a=$(modifyButtonName);
var d=$(saveButtonName);var b=$A(arguments);if(a){a.oldOnclick=a.onclick;a.onclick=modifyResetClick;a.form.keepFields=b;a.value=cancelLabel;
a=null}if(d){enableField(d);d=null}processFields(this,b,enableField)}function disableFormFields(){var a=$(modifyButtonName);
var e=$(saveButtonName);var d=$(backButtonName);if(a){if(a.oldOnclick){a.onclick=a.oldOnclick;a.value=modifyLabel;a.oldOnclick=null
}}if(e){disableField(e)}var b=$A(arguments);if(a){b.push(elementId(a))}if(d){b.push(elementId(d))}callback=function(f){disableField(f,false,true)
