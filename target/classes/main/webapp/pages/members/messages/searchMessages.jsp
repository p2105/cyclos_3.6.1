...
<c:choose><c:when test="${empty messages}">
	<div class="footerNote"><bean:message key="message.search.noResults"/></div>
</c:when><c:otherwise>
								...
								<c:choose><c:when test="${cyclos:name(msg.direction) == 'INCOMING'}">
									...
									<c:choose>
										...
										<c:when test="${msg.fromAMember}">
											...
											<a class="profileLink" memberId="${member.id}"><cyclos:truncate value="${member[memberProperty]}" length="50"/></a>
										</c:when>
