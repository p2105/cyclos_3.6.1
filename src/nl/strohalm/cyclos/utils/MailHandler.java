...
public class MailHandler implements InitializingBean, DisposableBean {
    ...
    public InternetAddress getInternetAddress(final Element element) {
        if (element == null || StringUtils.isEmpty(element.getEmail())) {
            return null;
        }
        final LocalSettings localSettings = settingsService.getLocalSettings();
        try {
            return new InternetAddress(element.getEmail(), element.getName(), localSettings.getCharset());
    ...
    public InternetAddress getInternetAddress(final PendingMember pendingMember) {
        if (pendingMember == null || StringUtils.isEmpty(pendingMember.getEmail())) {
            return null;
        }
        final LocalSettings localSettings = settingsService.getLocalSettings();
        try {
            return new InternetAddress(pendingMember.getEmail(), pendingMember.getName(), localSettings.getCharset());
        } catch (final UnsupportedEncodingException e) {
            // Shouldn't happen!!!
            return null;
        }
     ...
     */
    public String processBody(final String body, final Member member, final MessageCategory category, final boolean isHtml) {
        final LocalSettings localSettings = settingsService.getLocalSettings();
        ...
        variables.put("link", getRootUrl(isHtml, true));
     ...
     */
    public String processSubject(final String subject) {
        final LocalSettings localSettings = settingsService.getLocalSettings();
        final MessageSettings messageSettings = settingsService.getMessageSettings();

        String prefix = StringUtils.trimToEmpty(messageSettings.getMessageMailSubjectPrefix());
        final Map<String, String> variables = new HashMap<String, String>();
        variables.put("link", getRootUrl(false, true));
        variables.put("system_name", localSettings.getApplicationName());
    ...
    public void sendActivation(final boolean threaded, final Member member, final String password) {
        ...
        variableValues.put("link", getRootUrl(true, false));
        variableValues.put("url", getRootUrl(true, true));
    ...
    public void sendEmailValidation(final PendingMember pendingMember) {
        ...
        variableValues.put("link", linkGenerator.generateLinkForMailValidation(key));
        variableValues.put("url", linkGenerator.getMailValidationUrl(key));
    ...
    public void sendInvitation(final Element fromElement, final String toMail) {
        ...
        variableValues.put("system_name", localSettings.getApplicationName());
        variableValues.put("link", getRootUrl(true, true));
    ...
    private String getRootUrl(final boolean isHtml, final boolean useUrlAsLink) {
        if (linkGenerator == null) {
            return "";
        }
        final String rootUrl = linkGenerator.getRootUrl();
        if (isHtml) {
            if (useUrlAsLink) {
                return "<a href='" + rootUrl + "'>" + rootUrl + "</a>";
            } else {
                return linkGenerator.generateForApplicationRoot();
            }
        } else {
            return rootUrl;
        }
