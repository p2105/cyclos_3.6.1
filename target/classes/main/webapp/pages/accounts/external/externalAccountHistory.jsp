...
<c:choose><c:when test="${empty externalTransfers}">
	<div class="footerNote"><bean:message key="externalAccountHistory.search.noResults"/></div>
</c:when><c:otherwise>
			...
			<td colspan="2" align="left">
				<table class="defaultTable">
					<tr>
						...
						<th class="tdHeaderContents" width="15%"><bean:message key="externalTransfer.amount"/></th>
					...
					<c:forEach var="externalTransfer" items="${externalTransfers}">
						<c:choose> 
							<c:when test="${cyclos:name(externalTransfer.status) == 'PENDING' && externalTransfer.complete}">
								<c:set var="statusImage" value="pending.gif"/>
								<c:set var="tooltip"><cyclos:escapeJS><bean:message key="externalTransfer.status.COMPLETE_PENDING"/></cyclos:escapeJS></c:set>
								<c:set var="editableTransfer" value="true"/>
							</c:when>
							<c:when test="${cyclos:name(externalTransfer.status) == 'PENDING' && !externalTransfer.complete}">
								<c:set var="statusImage" value="incomplete.gif"/>
								<c:set var="tooltip"><cyclos:escapeJS><bean:message key="externalTransfer.status.INCOMPLETE_PENDING"/></cyclos:escapeJS></c:set>
								<c:set var="editableTransfer" value="true"/>
							</c:when>
							<c:when test="${cyclos:name(externalTransfer.status) == 'CHECKED'}">
								<c:set var="statusImage" value="checked.gif"/>
								<c:set var="tooltip"><cyclos:escapeJS><bean:message key="externalTransfer.status.CHECKED"/></cyclos:escapeJS></c:set>
								<c:set var="editableTransfer" value="false"/>
							</c:when>
							<c:otherwise>
								<c:set var="statusImage" value="conciliated.gif"/>
								<c:set var="tooltip"><cyclos:escapeJS><bean:message key="externalTransfer.status.PROCESSED"/></cyclos:escapeJS></c:set>
								<c:set var="editableTransfer" value="false"/>
							</c:otherwise>
						</c:choose>
