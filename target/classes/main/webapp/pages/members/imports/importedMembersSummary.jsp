        ...
        <td colspan="2" align="left" class="tdContentTableForms">
            <table class="defaultTable">
                <tr>
                    ...
                    <td>${memberImport.group.name}</td>
                </tr>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.accountType"/></td>
                    <td>${empty memberImport.accountType ? emptyAccountType : memberImport.accountType.name}</td>
                </tr>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.initialCreditTransferType"/></td>
                    <td>${empty memberImport.initialCreditTransferType ? emptyCreditType : memberImport.initialCreditTransferType.name}</td>
                </tr>
                <c:if test="${not empty memberImport.initialCreditTransferType}">
	                <tr>
	                    ...
	                    <td>${summary.credits.count} / <cyclos:format number="${summary.credits.amount}" unitsPattern="${memberImport.accountType.currency.pattern}" /></td>
	                </tr>
                </c:if>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.initialDebitTransferType"/></td>
                    <td>${empty memberImport.initialDebitTransferType ? emptyDebitType : memberImport.initialDebitTransferType.name}</td>
                </tr>
                <c:if test="${not empty memberImport.initialDebitTransferType}">
	                <tr>
	                    ...
	                    <td>${summary.debits.count} / <cyclos:format number="${summary.debits.amount}" unitsPattern="${memberImport.accountType.currency.pattern}" /></td>
	                </tr>
                </c:if>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.totalMembers"/></td>
                    <td><a id="totalLink" class="default">${summary.total}</a></td>
                </tr>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.successfulMembers"/></td>
                    <td><a id="successLink" class="default">${summary.total - summary.errors}</a></td>
                </tr>
                <tr>
                    <td class="headerLabel"><bean:message key="memberImport.membersWithErrors"/></td>
                    <td><a id="errorLink" class="default">${summary.errors}</a></td>
                </tr>
                <c:if test="${canImport}">
	                <tr>
	                    <td class="headerLabel"><bean:message key="memberImport.sendActivationMail"/></td>
	                    <td><html:checkbox property="sendActivationMail" value="true" /></td>
	                </tr>
                </c:if>
                <tr>
