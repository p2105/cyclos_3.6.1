...
public class MessageDAOImpl extends BaseDAOImpl<Message> implements MessageDAO {
    ...
    public void assignPendingToSendByBroker(final Message message, final Member broker) {
        ...
        runNative(new JDBCCallback() {
            ...
            public void execute(final JDBCWrapper jdbc) throws SQLException {
                ...
                sql.append(" insert into messages_pending_to_send");
                sql.append(" (message_id, member_id)");
                sql.append(" select ?, id");
                sql.append(" from members m");
                sql.append(" where m.member_broker_id = ?");
                jdbc.execute(sql.toString(), message.getId(), broker.getId());
            }
        });
    }
    ...
    @Override
    public void markAsSent(final Message message, final Member member) {
        runNative(new JDBCCallback() {
            @Override
            public void execute(final JDBCWrapper jdbc) throws SQLException {
                jdbc.execute("delete from messages_pending_to_send where message_id = ? and member_id = ?", message.getId(), member.getId());
            }
        });
    }

    @Override
    public Member nextMemberToSend(final Message message) {
        Session session = getSession();
        SQLQuery query = session.createSQLQuery("select member_id from messages_pending_to_send where message_id = :id");
        ...
        return (Member) session.load(Member.class, list.get(0));
    }

    @Override
    public Message nextToSend() {
        return uniqueResult("from Message m where m.finished = false", null);
    }

    @Override
    public void removeAllPending(final Message message) {
        runNative(new JDBCCallback() {
            @Override
            public void execute(final JDBCWrapper jdbc) throws SQLException {
                jdbc.execute("delete from messages_pending_to_send where message_id = ?", message.getId());
            }
        });
    }
