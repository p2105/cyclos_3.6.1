...
change the name, the description and the &quot;access settings&quot;.
The status can only be defined on group creation.
...
<ul>
	...
	This setting is used if you want to access Cyclos from within a website. 
	The settings works the same as the global container page 
	(see <a href="${pagePrefix}settings#local"><u>Settings - Local settings</u></a> but just for this group. 
...
<ul>
	...
	validation is required. After submitting the user will receive a
	...
	period has not been expired the members are in a pending status. The
	maxim confirmation period can be defined at the &quot;limits&quot;
	section of the local settings <a href="${pagePrefix}settings#local"><u>help
	file</u></a> The pending members can be view at the <a
		href="${pagePrefix}user_management#search_pending_member"><u>Pending
	members</u></a> page.<br>
	The following validation options are available:
	<ul>
		<li><b>Do not validate:</b> The e-mail is not validated. The
		member will be directly part of the initial group that is set for the
		public registration. (or, in case of a registration by admin/broker
		part of the group selected by the admin or broker).
		<li><b>Validate on public registration only:</b> The registration
		is only validated when members register at the public registration
		page.
		<li><b>Validate on public and broker registrations:</b> The
		registration is validated when members register at the public
		registration page but also when brokers register a member.
		<li><b>Validate on all registrations (admin / broker /
		public):</b> (Self explanatory)
	</ul>
...
<ul>
	...
	permissions in Ads.
		...
		can change their login name in their profile. 
		<li><b>Change own user name:</b>  
	</ul>
