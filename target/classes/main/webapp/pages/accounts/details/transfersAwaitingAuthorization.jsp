					...
					<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
						...
						<td align="left">
							<c:choose><c:when test="${transfer.fromSystem}">
								...
								<a class="profileLink" memberId="${transfer.from.member.id}">${transfer.from.ownerName}</a>
						...
						<c:if test="${isAdmin || isBroker}">
							<td align="left">
								<c:choose><c:when test="${transfer.toSystem}">
									...
									<a class="profileLink" memberId="${transfer.to.member.id}">${transfer.to.ownerName}</a>
								</c:otherwise></c:choose>
							</td>
						</c:if>
