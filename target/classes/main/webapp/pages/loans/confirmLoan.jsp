        ...
        <td colspan="2" align="left" class="tdContentTable">
            <table class="defaultTable">
				...
				<tr>
					...
					<td>${loan.transferType.name}</td>
				</tr>
				<c:choose><c:when test="${empty loan.loanGroup}">
					<tr>
						<td class="headerLabel"><bean:message key="member.member"/></td>
						<td><a class="profileLink default" memberId="${loan.member.id}">${loan.member.username} - ${loan.member.name}</a></td>
					</tr>
				</c:when><c:otherwise>
					<tr>
						<td class="headerLabel"><bean:message key='loan.group'/></td>
						<td>${loan.loanGroup.name}</td>
					</tr>
					<tr>
						...
						<td>
							<c:forEach var="member" varStatus="status" items="${membersInGroup}">
								...
								<a class="profileLink default" memberId="${member.id}">${member.username} - ${member.name}</a>
				...
				<c:if test="${not empty loan.date}">
					<tr>
						...
						<td><cyclos:format rawDate="${loan.date}"/></td>
					</tr>
				</c:if>
				<tr>
			        ...
			        <c:if test="${not empty value.value}">
			            <tr>
			                <td class="headerLabel">${field.name}</td>
			   				<td nowrap="nowrap"><cyclos:customField field="${field}" value="${value}" textOnly="true"/></td>
						</tr>
					</c:if>
			    </c:forEach>
				...
				<tr>
					...
					<td><cyclos:escapeHTML>${loan.description}</cyclos:escapeHTML></td>
				</tr>
