		    	  	...
		    	  	<table class="defaultTable">
						...
						<c:forEach var="po" items="${paymentObligations}">
							<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
								...
								<c:if test="${showBuyer}">										            		
									<td align="left" title="<cyclos:escapeHTML value='${po.buyer.name}'/>"><a class="profileLink" memberId="${po.buyer.id}">${po.buyer.username}</a></td>
								</c:if>
								<c:if test="${showSeller}">										            		
									<td align="left" title="<cyclos:escapeHTML value='${po.seller.name}'/>"><a class="profileLink" memberId="${po.seller.id}">${po.seller.username}</a></td>
								</c:if>
