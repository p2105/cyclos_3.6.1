...
<c:choose><c:when test="${empty brokerings}">
	<div class="footerNote"><bean:message key="brokering.list.noResults"/></div>
</c:when><c:otherwise>
	<table class="defaultTableContent" cellspacing="0" cellpadding="0">
	    <tr>
	        ...
	        <td class="tdHelpIcon" align="right" width="13%" valign="middle" nowrap="nowrap">
		        ...
		        <cyclos:help page="${isAdmin ? 'brokering#admin_brokering_list' : isBroker ? 'brokering#broker_search_member_result' : 'user_management#search_member_result'}" td="false"/>
	        </td>
	    </tr>
	    <tr>
	            ...
	            <table class="defaultTable" cellspacing="0" cellpadding="0">
					...
					<c:forEach var="brokering" items="${brokerings}">
		                ...
		                <tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
		                    <td align="left"><a class="profileLink" memberId="${member.id}">${member.username}</a></td>
		                    <td align="left"><a class="profileLink" memberId="${member.id}">${member.name}</a></td>
			                <c:if test="${canChangeBroker}">
