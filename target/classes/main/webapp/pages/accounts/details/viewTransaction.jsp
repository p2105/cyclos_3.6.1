            	...
            	<tr>
            		...
            		<td>
            			...
            			<c:choose><c:when test="${transfer.actuallyFromSystem}">
            				...
            				<a style="text-decoration:underline;" class="profileLink default" memberId="${from.owner.id}">${from.owner.username} - ${from.owner.name}</a>
            			</c:otherwise></c:choose>
            		</td>
            	</tr>
            	<tr>
            		...
            		<td>
            			...
            			<c:choose><c:when test="${transfer.actuallyToSystem}">
            				...
            				<a style="text-decoration:underline;" class="profileLink default" memberId="${to.owner.id}">${to.owner.username} - ${to.owner.name}</a>
            			</c:otherwise></c:choose>
            		</td>
            	</tr>
            	<c:if test="${showBy}">
	            		...
	            		<td>
	            			<c:choose>
	            				<c:when test="${not empty byMember}">
	    	        				<a style="text-decoration:underline;" class="profileLink default" memberId="${byMember.id}">${byMember.username} - ${byMember.name}</a>
		            			</c:when>
	            				<c:when test="${not empty byOperator}">
	    	        				<a style="text-decoration:underline;" class="operatorProfileLink default" operatorId="${byOperator.id}">${byOperator.username} - ${byOperator.name}</a>
		            			</c:when>
		            			<c:when test="${not empty byAdmin}">
		            				<a style="text-decoration:underline;" class="adminProfileLink default" adminId="${byAdmin.id}">${byAdmin.username} - ${byAdmin.name}</a>
		            			</c:when>
	        ...
	        <td colspan="2" align="left" class="tdContentTableLists">
	            <table class="defaultTable">
					...
					<c:forEach var="auth" items="${authorizations}">
						<c:set var="byNature" value="${cyclos:name(auth.by.nature)}"/>
						<c:choose>
							<c:when test="${byNature == 'MEMBER'}">
								<c:set var="linkClass" value="profileLink"/>
								<c:set var="idAttribute" value="memberId"/>
							</c:when>
							<c:when test="${byNature == 'ADMIN'}">
								<c:set var="linkClass" value="adminProfileLink"/>
								<c:set var="idAttribute" value="adminId"/>
							</c:when>
						</c:choose> 
						<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
							...
							<td align="center"><a class="default ${linkClass}" ${idAttribute}="${by.id}">${auth.by.username}</a></td>
