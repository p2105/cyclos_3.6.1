            ...
            <table class="defaultTable">
            	<tr>
               	...
               	<c:if test="${activities.showReferencesInformation}">
					...
					<c:forEach var="level" items="${referenceLevels}">
		            	<tr>
							...
							<td width="20%"><cyclos:format number="${givenReferences[level]}" /></td>
							<td width="30%" class="headerLabel"><bean:message key="reference.level.${level}"/></td>
							<td width="20%"><cyclos:format number="${receivedReferences[level]}" /></td>
		                </tr>
					</c:forEach>
				</c:if>
               	...
               	<c:if test="${activities.showAdsInformation}">
	            	...
	            	<tr>
						...
						<td><cyclos:format number="${activities.adsByStatus[adStatus['ACTIVE']]}" /></td>
						<c:if test="${activities.showNonActiveAdsInformation}">
							<td class="headerLabel"><bean:message key="ad.status.EXPIRED"/></td>
							<td><cyclos:format number="${activities.adsByStatus[adStatus['EXPIRED']]}" /></td>
						</c:if>
	                </tr>
					<c:if test="${activities.showNonActiveAdsInformation}">
		            	<tr>
							<td class="headerLabel"><bean:message key="ad.status.PERMANENT"/></td>
							<td><cyclos:format number="${activities.adsByStatus[adStatus['PERMANENT']]}" /></td>
							<td class="headerLabel"><bean:message key="ad.status.SCHEDULED"/></td>
							<td><cyclos:format number="${activities.adsByStatus[adStatus['SCHEDULED']]}" /></td>
		                </tr>
		            </c:if>
	            </c:if>
				...
				<c:if test="${activities.showInvoicesInformation}">
	            	...
	            	<tr>
						...
						<td><cyclos:format number="${activities.outgoingInvoices.count}" /></td>
						<td width="30%" class="headerLabel"><bean:message key="activities.invoices.outgoing.amount"/></td>
						<td><cyclos:format unitsPattern="${unitsPattern}" number="${activities.outgoingInvoices.amount}"/></td>
	                </tr>
	               	<tr>
						<td width="30%" class="headerLabel"><bean:message key="activities.invoices.incoming.count"/></td>
						<td><cyclos:format number="${activities.incomingInvoices.count}" /></td>
						<td width="30%" class="headerLabel"><bean:message key="activities.invoices.incoming.amount"/></td>
						<td><cyclos:format unitsPattern="${unitsPattern}" number="${activities.incomingInvoices.amount}"/></td>
	                </tr>
	            </c:if>
			</table>
		</td>
	        	...
	        	<td align="left" class="tdContentTableForms" colspan="2">
	        		<cyclos:layout columns="4" className="defaultTable">
						<c:if test="${activities.showAccountInformation && accountActivities.showAccountInfo}">
            				...
            				<cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountStatus.balance}"/></cyclos:cell>
                   			<cyclos:rowBreak/>
						</c:if>
						...
						<c:if test="${activities.showAccountInformation && accountActivities.showAccountInfo}">
	                       	<c:if test="${accountStatus.creditLimit > 0}">
	                           	...
	                           	<cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${-accountStatus.creditLimit}"/></cyclos:cell>
	                       	</c:if>
	                       	<c:if test="${accountStatus.upperCreditLimit > 0}">
	                           	...
	                           	<cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountStatus.upperCreditLimit}"/></cyclos:cell>
	                       	</c:if>
		            		...
		            		<cyclos:cell><cyclos:format number="${accountActivities.remainingLoans.count}" /></cyclos:cell>
		            		<cyclos:cell className="headerLabel"><bean:message key="activities.loans.amount"/></cyclos:cell>
		            		<cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.remainingLoans.amount}"/></cyclos:cell>
		                    ...
		                    <cyclos:cell><cyclos:format number="${accountActivities.debitsLast30Days.count}" /></cyclos:cell>
		                    <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageBuy"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.debitsLast30Days.average}"/></cyclos:cell>
	                        
	                        <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.numberSell"/></cyclos:cell>
		                    <cyclos:cell><cyclos:format number="${accountActivities.creditsLast30Days.count}" /></cyclos:cell>
	                        <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageSell"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.creditsLast30Days.average}"/></cyclos:cell>
		                    ...
		                    <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.numberBuy"/></cyclos:cell>
		                    <cyclos:cell><cyclos:format number="${accountActivities.debitsAllTime.count}" /></cyclos:cell>
							<cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageBuy"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.debitsAllTime.average}"/></cyclos:cell>
	
		                    <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.numberSell"/></cyclos:cell>
		                    <cyclos:cell><cyclos:format number="${accountActivities.creditsAllTime.count}" /></cyclos:cell>
	                        <cyclos:cell className="headerLabel"><bean:message key="activities.transactions.averageSell"/></cyclos:cell>
	                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.creditsAllTime.average}"/></cyclos:cell>
		                    ...
		                    <c:if test="${member.group.broker}">
			                    ...
			                    <cyclos:cell><cyclos:format number="${accountActivities.brokerCommission.count}" /></cyclos:cell>
		                        <cyclos:cell className="headerLabel"><bean:message key="activities.brokering.commission.amount"/></cyclos:cell>
		                        <cyclos:cell><cyclos:format unitsPattern="${unitsPattern}" number="${accountActivities.brokerCommission.amount}"/></cyclos:cell>
			                </c:if>
		                </c:if>
	    			</cyclos:layout>
	    		</td>
