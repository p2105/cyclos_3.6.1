	    	  	...
	    	  	<table class="defaultTable">
					...
					<c:forEach var="certificationDTO" items="${listCertificationDTOs}">
						<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
							...
							<c:if test="${hasViewPermission || isIssuer}">
								<td align="left" title="<cyclos:escapeHTML value='${certificationDTO.certification.buyer.name}'/>"><a class="profileLink" memberId="${certificationDTO.certification.buyer.id}">${certificationDTO.certification.buyer.username}</a></td>
							</c:if>
							<c:if test="${not isIssuer}">
								<td align="left" title="<cyclos:escapeHTML value='${certificationDTO.certification.issuer.name}'/>"><a class="profileLink" memberId="${certificationDTO.certification.issuer.id}">${certificationDTO.certification.issuer.username}</a></td>
							</c:if>
