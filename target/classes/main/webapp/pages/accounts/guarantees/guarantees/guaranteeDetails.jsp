        ...
        <td colspan="4" align="left" class="tdContentTableForms">
        	<table class="defaultTable">
	            <tr>
	            	...
	            	<td nowrap="nowrap" colspan="3"><bean:message key="guarantee.status.${guarantee.status}"/></td>
	            </tr>
	            <tr>
	            	<td class="headerLabel"><bean:message key="guarantee.guaranteeType" /></td>
	            	<td colspan="3">${guarantee.guaranteeType.name}</td>	            	
	            </tr>
			        ...
			        <c:if test="${not empty value.value}">
			            <tr>
			                <td class="headerLabel">${field.name}</td>
			   				<td colspan="3"><cyclos:customField field="${field}" value="${value}" textOnly="true"/></td>
						</tr>
					</c:if>
			    </c:forEach>
				<tr>
	            	<td class="headerLabel"><bean:message key="guarantee.amount" /></td>
	            	<td colspan="3"><cyclos:format number="${guarantee.amount}" unitsPattern="${guarantee.guaranteeType.currency.pattern}" /></td>
	            </tr>
        		<tr>
        			<td class="headerLabel"><bean:message key="guarantee.issuerName" /></td>
	            	<td colspan="3"><a class="profileLink" memberId="${guarantee.issuer.id}"><cyclos:escapeHTML value="${guarantee.issuer.name}"/></a></td>
				</tr>
				<tr>
        			<td class="headerLabel"><bean:message key="guarantee.buyerName" /></td>
	            	<td colspan="3"><a class="profileLink" memberId="${guarantee.buyer.id}"><cyclos:escapeHTML value="${guarantee.buyer.name}"/></a></td>
	            </tr>
				<c:if test="${not isWithBuyerOnly}">
	        		<tr>
						<td class="headerLabel"><bean:message key="guarantee.sellerName" /></td>
		            	<td colspan="3"><a class="profileLink" memberId="${guarantee.seller.id}"><cyclos:escapeHTML value="${guarantee.seller.name}"/></a></td>
		            </tr>
				</c:if>
	            <tr>
	            	<td class="headerLabel"><bean:message key="guarantee.registrationDate" /></td>
	            	<td colspan="3"><cyclos:format dateTime='${guarantee.registrationDate}'/></td>
	            </tr>
	            <tr>
	            ...
	            <tr>
					...
					<c:if test="${showCurrentFeeValues}">
						...
						<td align="left"><cyclos:format number="${guarantee.creditFee}" unitsPattern="${guarantee.guaranteeType.currency.pattern}"/></td>
					</c:if>
       			</tr>
				<tr>
					...
					<c:if test="${showCurrentFeeValues}">
						...
						<td align="left"><cyclos:format number="${guarantee.issueFee}" unitsPattern="${guarantee.guaranteeType.currency.pattern}"/></td>
					</c:if>
				</tr>
				<c:if test="${showLoan}">
					<tr>
						<td class="headerLabel"><bean:message key="guarantee.generatedLoan"/></td>
					    <td><a id="loanLink" class="default" guaranteeId="${guarantee.id}" loanId="${guarantee.loan.id}"><bean:message key='guarantee.loan'/></a></td>				
					</tr>
				</c:if>
				<tr>
	    	  	...
	    	  	<table class="defaultTable">
					...
					<c:forEach var="log" items="${guarantee.logs}" varStatus="idx">
						<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
							...
							<c:choose>
								<c:when test="${logsBy[idx.index].byType == 'Member'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="profileLink" memberId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a>
								</c:when>
								<c:when test="${logsBy[idx.index].byType == 'Operator'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="operatorProfileLink" operatorId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a>
								</c:when>
								<c:when test="${logsBy[idx.index].byType == 'Admin'}">
									<td align="left" title="<cyclos:escapeHTML value="${logsBy[idx.index].by.name}"/>"><a class="adminProfileLink" adminId="${logsBy[idx.index].by.id}">${logsBy[idx.index].by.username}</a>
								</c:when>
