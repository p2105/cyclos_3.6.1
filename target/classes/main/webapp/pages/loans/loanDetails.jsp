	...
	<tr>
		...
		<cyclos:layout columns="4">
			<c:choose><c:when test="${loan.toGroup}">
				...
				<cyclos:cell width="30%">${loan.loanGroup.name}</cyclos:cell>
			</c:when><c:otherwise>
				<cyclos:cell className="headerLabel" width="20%"><bean:message key='member.username'/></cyclos:cell>
				<cyclos:cell width="30%">${loan.member.username}</cyclos:cell>
			...
			<c:choose><c:when test="${loan.toGroup}">
				...
				<cyclos:cell>${loan.member.username}</cyclos:cell>
				<cyclos:cell className="headerLabel"><bean:message key='member.name' /></cyclos:cell>
				<cyclos:cell>${loan.member.name}</cyclos:cell>
			</c:when><c:otherwise>
			...
			<cyclos:cell><cyclos:format date="${loan.grantDate}"/></cyclos:cell>
			
			<cyclos:cell className="headerLabel"><bean:message key='transfer.type'/></cyclos:cell>
			<cyclos:cell>${loan.transfer.type.name}</cyclos:cell>
			<cyclos:cell className="headerLabel"><bean:message key='loan.expirationDate' /></cyclos:cell>
			<cyclos:cell><cyclos:format rawDate="${loan.expirationDate}"/></cyclos:cell>

			<cyclos:cell className="headerLabel"><bean:message key='loan.amount' /></cyclos:cell>
			<cyclos:cell><cyclos:format number="${loan.amount}" unitsPattern="${currencyPattern}" /></cyclos:cell>
			<c:choose>
				<c:when test="${cyclos:name(loan.status) == 'CLOSED'}">
					...
					<cyclos:cell><cyclos:format dateTime="${loan.repaymentDate}"/></cyclos:cell>
				</c:when>
				<c:when test="${cyclos:name(loan.status) == 'OPEN'}">
					...
					<cyclos:cell><cyclos:format number="${loan.remainingAmount}" unitsPattern="${currencyPattern}"/></cyclos:cell>
				</c:when>
			</c:choose>
			...
			<c:if test="${cyclos:name(parameters.type) == 'WITH_INTEREST'}">
				<c:if test="${loan.amount != loan.totalAmount}">
					...
					<cyclos:cell><cyclos:format number="${loan.totalAmount}" unitsPattern="${currencyPattern}"/></cyclos:cell>
				</c:if>
		    	<c:if test="${not empty parameters.monthlyInterest}">
		            <cyclos:cell className="headerLabel"><bean:message key='loan.monthlyInterest'/></cyclos:cell>
		   			<cyclos:cell><cyclos:format amount="${parameters.monthlyInterestAmount}" unitsPattern="${monthlyInterestPattern}"/></cyclos:cell>
		    	</c:if>
		    	<c:if test="${not empty parameters.grantFee}">
		            <cyclos:cell className="headerLabel"><bean:message key='loan.grantFee'/></cyclos:cell>
		   			<cyclos:cell><cyclos:format amount="${parameters.grantFee}" unitsPattern="${grantFeePattern}"/></cyclos:cell>
		    	</c:if>
		    	<c:if test="${not empty parameters.expirationFee}">
		            <cyclos:cell className="headerLabel"><bean:message key='loan.expirationFee'/></cyclos:cell>
		   			<cyclos:cell><cyclos:format amount="${parameters.expirationFee}" unitsPattern="${expirationFeePattern}"/></cyclos:cell>
		    	</c:if>
		    	<c:if test="${not empty parameters.expirationDailyInterest}">
		            <cyclos:cell className="headerLabel"><bean:message key='loan.expirationDailyInterest'/></cyclos:cell>
		   			<cyclos:cell><cyclos:format amount="${parameters.expirationDailyInterestAmount}" unitsPattern="expirationDailyInterestPattern"/></cyclos:cell>
		    	</c:if>
		    </c:if>
		    ...
		    <c:forEach var="entry" items="${customFields}">
		        ...
		        <c:if test="${not empty value.value}">
		            <cyclos:cell className="headerLabel">${field.name}</cyclos:cell>
		   			<cyclos:cell><cyclos:customField field="${field}" value="${value}" textOnly="true"/></cyclos:cell>
	   			</c:if>
		    </c:forEach>
			...
			<cyclos:cell colspan="3"><cyclos:escapeHTML>${loan.transfer.description}</cyclos:escapeHTML></cyclos:cell>
		...
		<tr>
			...
			<table class="defaultTable">
				...
				<c:forEach var="member" items="${membersInLoan}">
					<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
						<td align="center"><a class="${member == loan.member ? 'fieldDecoration' : ''} profileLink" memberId="${member.id}">${member.username}</a></td>
						<td align="center">
							<a class="${member == loan.member ? 'fieldDecoration' : ''} profileLink" memberId="${member.id}">${member.name}</a>
						</td>
					</tr>
				</c:forEach>
			</table>
			...
			<td colspan="2" align="left" class="tdContentTableForms">
				...
				<table class="defaultTable">
					<c:if test="${canRepay or canDiscard}">				
						<c:if test="${cyclos:name(parameters.type) != 'SINGLE_PAYMENT'}">
							<tr>
								<td class="label" width="40%"><bean:message key='loan.repayment.paymentNumber' /></td>
								<td>
						...
						<tr>
							<td class="label" width="40%"><bean:message key='loan.repayment.amountToRepaid'/></td>
