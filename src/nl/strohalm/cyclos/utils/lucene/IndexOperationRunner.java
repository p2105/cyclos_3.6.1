...
public class IndexOperationRunner implements Runnable, InitializingBean, DisposableBean {
        ...
        try {
            ...
            Document document = readonlyTransactionTemplate.execute(new TransactionCallback<Document>() {
                ...
                public Document doInTransaction(final TransactionStatus status) {
                    try {
                        Indexable entity = (Indexable) getSession().load(entityType, id);
            ...
            if (document != null) {
                writer.updateDocument(new Term("id", document.get("id")), document, getAnalyzer());
     ...
     */
    private void optimize(final Class<? extends Indexable> entityType) {
        final IndexWriter writer = getWriter(entityType);
        try {
            writer.optimize();
            writer.commit();
            createAlert(SystemAlert.Alerts.INDEX_OPTIMIZED, entityType);
        } catch (CorruptIndexException e) {
            handleIndexCorrupted(entityType);
        } catch (final Exception e) {
    ...
    private void rollback(final Class<? extends Indexable> entityType, final IndexWriter writer) {
        if (writer == null) {
            return;
        }
        try {
