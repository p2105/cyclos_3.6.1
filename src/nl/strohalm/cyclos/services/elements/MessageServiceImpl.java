...
import nl.strohalm.cyclos.services.elements.exceptions.PendingMessagesException;
...
public class MessageServiceImpl implements MessageServiceLocal, DisposableBean, InitializingService {
    ...
    public Member nextMemberToSend(final Message message) {
        return messageDao.nextMemberToSend(message);
    }
        ...
        for (final Long id : ids) {
            ...
            if (action == MessageAction.DELETE) {
                try {
                    ...
                    throw new PendingMessagesException();
                }
            } else {
                switch (action) {
                    case MOVE_TO_TRASH:
    ...
    @Override
    public void sendReceiverCopy(final Message message, final Member member) {
        ...
        toSend.setCategory(message.getCategory());
        toSend.setSubject(message.getSubject());
        toSend.setBody(message.getBody());
        toSend.setHtml(message.isHtml());
        final Set<MessageChannel> deliveredChannels = send(toSend, null, null, null);
    ...
    private Set<MessageChannel> send(final Message message, final String smsMessage, final String smsTraceData, Set<MessageChannel> messageChannels) {
        ...
        if (toMember != null) {
            ...
            if (messageChannels.contains(MessageChannel.EMAIL) && StringUtils.isNotEmpty(email)) {
                final InternetAddress replyToAddress = LoggedUser.hasUser() ? mailHandler.getInternetAddress(LoggedUser.element()) : null;
