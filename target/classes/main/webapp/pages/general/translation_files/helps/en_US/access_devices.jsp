...
<br><br><br>An access device is a payment medium
...
have more than one cards but only one active card.


<span class="broker admin"> It is possible to require in stead of
the PIN the transaction or login password of the user.<br>
A card is based on a card type which is a kind of a template for the card.
</span>
...
In order to generate a card a <a
	href="${pagePrefix}access_devices#list_card_type"><u>card type</u></a> needs to exist 
and it it needs to be assigned it to one or more a member or broker groups. After
...
<h3>Card types</h3>
This window shows a list with all card types in the system. A card type
...
At this page you can create or edit a 
<a
href="${pagePrefix}access_devices#list_card_type"><u>card type</u></a>.
A card type can only be edited when there are no cards generated with that card type.<br>
...
<ul>
	...
	&quot;####&quot; &quot;##/##&quot;<br></div>
	It is possible to put a real (fixed) number like &quot;123&quot;. 	
	...
	<ul>
		...
		<li><b>Manual</b>: The member and broker/admin (with permissions)
...
at a computer (or any device with a web browser). A POS normally is located at a
...
(Not all actions might be available. The actions depend on permissions)
<ul>
