		...
		<tr>
			<td colspan="2" align="left" class="bordered tdContentTableLists" style="border-top:none">
				<cyclos:layout columns="4" className="defaultTable">
					...
					<cyclos:cell width="30%"><cyclos:format number="${status.balance}" unitsPattern="${unitsPattern}" /></cyclos:cell>
					<c:if test="${status.creditLimit > 0 or status.reservedAmount > 0}">
						...
						<cyclos:cell width="30%"><cyclos:format number="${status.availableBalance}" unitsPattern="${unitsPattern}" /></cyclos:cell>
					</c:if>
					<c:if test="${status.creditLimit > 0 and status.creditLimit != null}">
						...
						<cyclos:cell width="30%"><cyclos:format number="${status.creditLimit}" unitsPattern="${unitsPattern}" /></cyclos:cell>
					</c:if>
					<c:if test="${status.reservedAmount > 0 and status.creditLimit != null}">
						...
						<cyclos:cell width="30%"><cyclos:format number="${status.reservedAmount}" unitsPattern="${unitsPattern}" /></cyclos:cell>
					</c:if>
					...
					<tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
						...
						<td align="left">
							<c:choose><c:when test="${empty entry.relatedMember}">
								...
								<a class="profileLink" memberId="${entry.relatedMember.id}"><cyclos:truncate value="${entry.relatedMember[memberProperty]}" length="50"/></a>
							</c:otherwise></c:choose>
						</td>
