...
<c:if test="${queryExecuted}">
	<c:choose><c:when test="${empty elements}">
		<div class="footerNote"><bean:message key="admin.search.noResults"/></div>
	</c:when><c:otherwise>
		            ...
		            <table class="defaultTable" cellspacing="0" cellpadding="0">
						...
						<c:forEach var="admin" items="${elements}">
			                <tr class="<t:toggle>ClassColor1|ClassColor2</t:toggle>">
			                    <td align="left" width="30%"><a class="adminProfileLink" adminId="${admin.id}">${admin.username}</a></td>
			                    <td align="left" width="70%"><a class="adminProfileLink" adminId="${admin.id}">${admin.name}</a></td>
			                </tr>
			            </c:forEach>
		            </table>
		        </td>
